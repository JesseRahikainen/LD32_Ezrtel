...
#define MAX_RENDER_INSTRUCTIONS 4096
typedef struct {
	Vector3 pos;
	Vector2 scaleSize;
	Color color;
...
int queueRenderImage_sv( int imgID, unsigned int camFlags, Vector2 startPos, Vector2 endPos, Vector2 startScale, Vector2 endScale, char depth )
{
	...
	vec2_HadamardProd( &endScale, &( ri->end.scaleSize ), &( ri->end.scaleSize ) );
	...
	float startRot, float endRot, Color startColor, Color endColor, char depth )
{
	...
	ri->start.rotation = startRot;
	ri->end.rotation = endRot;
	vec2_HadamardProd( &( ri->start.scaleSize ), &startScale, &( ri->start.scaleSize ) );
	vec2_HadamardProd( &( ri->end.scaleSize ), &endScale, &( ri->end.scaleSize ) );
...
int queueRenderImage_s_r_c( int imgID, unsigned int camFlags, Vector2 startPos, Vector2 endPos, float startScale, float endScale,
	float startRot, float endRot, Color startColor, Color endColor, char depth )
{
	RenderInstruction* ri = GetNextRenderInstruction( imgID, camFlags, startPos, endPos, depth );
	if( ri == NULL ) {
